...
def one_cycle(p, workflow_cfg, resource, rep_idx, iter_idx, resource_cfg):
    ...
    task1_output = ['open_autopsf.dx']
    task2_output = ['open_autopsf-grid.dx']
    task3_output = ['closed_autopsf-grid.pdb']
    task4_output = ['closed-extrabonds.txt', 'closed-extrabonds-cispeptide.txt', 'closed-extrabonds-chirality.txt']
    ...
    task1.cpu_reqs = {
            ...
            'process_type': None,
            'thread_type': None}
    task1_tcl_cmds = [ 'mol new open.pdb' ]
    task1_tcl_cmds += [ 'package require autopsf' ]
    task1_tcl_cmds += [ 'autopsf open.pdb' ]
    ...
    task3.cpu_reqs = {
            ...
            'process_type': None,
            'thread_type': None}
    task3_tcl_cmds = [ 'mol new closed.pdb' ]
    task3_tcl_cmds += [ 'package require autopsf' ]
    task3_tcl_cmds += [ 'autopsf closed.pdb' ]
    task3_tcl_cmds += [ 'package require mdff' ]
    task3_tcl_cmds += [ 'mdff gridpdb -psf closed_autopsf.psf -pdb closed_autopsf.pdb -o {}'.format(task3_output[0]) ]
    ...
    task4_tcl_cmds = [ 'package require ssrestraints',
            'mol new closed_autopsf.psf',
            'mol addfile closed_autopsf.pdb',
            'ssrestraints -psf closed_autopsf.psf -pdb closed_autopsf.pdb -o {} -hbonds'.format(task4_output[0]) ]
    ...
    task4.copy_input_data = ['$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, third_stage.uid, task3.uid, 'closed_autopsf.pdb'),
            '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, third_stage.uid, task3.uid, 'closed_autopsf.psf')]
    ...
    task5_tcl_cmds += [ 'mdff setup -o codh -psf closed_autopsf.psf ' \
            + '-pdb closed_autopsf.pdb ' \
            + '-griddx open_autopsf-grid.dx ' \
            + '-gridpdb closed_autopsf-grid.pdb ' \
            + '-extrab {closed-extrabonds.txt closed-extrabonds-cispeptide.txt closed-extrabonds-chirality.txt} ' \
            + f'-gscale {gscale} -minsteps {minsteps} -numsteps {numsteps}' ]
    
    task5.copy_input_data = [ '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, second_stage.uid, task2.uid, 'open_autopsf-grid.dx'),
            '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, third_stage.uid, task3.uid, 'closed_autopsf.pdb'),
            '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, third_stage.uid, task3.uid, 'closed_autopsf.psf'),
            '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, third_stage.uid, task3.uid, 'closed_autopsf-grid.pdb')]
    ...
    if resource[-4:] == "cuda":
        ...
        task6.arguments = ['+ignoresharing', '+ppn', cpus_per_ensemble, 'codh-step1.namd']
    ...
    task6.copy_input_data = [ '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, fifth_stage.uid, task5.uid, 'codh-step1.namd'),
        '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, fifth_stage.uid, task5.uid, 'closed_autopsf.psf'),
        '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, fifth_stage.uid, task5.uid, 'closed_autopsf.pdb'),
        '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, fifth_stage.uid, task5.uid, 'closed_autopsf-grid.pdb'),
        '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, fifth_stage.uid, task5.uid, 'open_autopsf-grid.dx'),
        '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, fifth_stage.uid, task5.uid, 'closed-extrabonds-chirality.txt'),
        '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, fifth_stage.uid, task5.uid, 'closed-extrabonds-cispeptide.txt'),
        '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, fifth_stage.uid, task5.uid, 'closed-extrabonds.txt'),
    ...
    if iter_idx != 0:
        task6.pre_exec += ['echo 0 > $RP_PILOT_STAGING/base_{}_cc.dat'.format(iter_idx - 1),
              ...
              #'touch codh-step${iter_idxpp}.namd',
              #'head -n 17 codh-step1.namd >> codh-step${iter_idxpp}.namd',
              #'echo "set OUTPUTNAME codh-step${iter_idxpp}" >> codh-step${iter_idxpp}.namd',
              #'echo "set INPUTNAME ${replica_id}_${iter_id}_codh-step{}" >> codh-step${iter_idxpp}.namd'.format(iter_idx),
              #'tail -n 23 codh-step1.namd >> codh-step${iter_idxpp}.namd'] 
              'sed -i -- "s/#####/set INPUTNAME ${replica_id}_${iter_id}_codh-step1\\n\\n#####/" codh-step1.namd']
    #task6.post_exec = ['if [ -f "codh-step${iter_idxpp}.dcd" ]; then ln -s codh-step${iter_idxpp}.dcd codh-step1.dcd; fi']
    # note: for now this remains but ideally should be changed. not changed because of subsequent stages
    task6.download_output_data = ['codh-step1.dcd']   
    ...
    task7.cpu_reqs = {
            ...
            'process_type': None,
            'thread_type': None}
    task7_tcl_cmds = [ 'mol new closed_autopsf.psf' ]
    task7_tcl_cmds += [ 'mol addfile codh-step1.dcd waitfor all' ]    # load the full mdff trajectory
    #task7_tcl_cmds += [ 'package require mdff',
    #                    'mdff check -ccc -map open_autopsf.dx -res {} waitfor -1 -cccfile all.cc.dat'.format(resolution)]
    ...
    task8.cpu_reqs = {
            ...
            'thread_type': None}
    if iter_idx != 0:
        task8_tcl_cmds = [ 'mol new closed_autopsf.psf' ]
        task8_tcl_cmds += ['mol addfile last_from_prev_iter.pdb']         # first frame is now the last_from_prev_iter
        task8_tcl_cmds += [ 'mol addfile codh-step1.dcd waitfor all' ]    # load the full mdff trajectory
        task8_tcl_cmds += [ 'set outfilename cc.dat',
                             'package require mdff',
                             'set selall [atomselect 0 "all"]',
                             '$selall frame 0',
                             'set lcc [mdff ccc $selall -i open_autopsf.dx -res {}]'.format(resolution),
                             '$selall frame last',
                             'set fcc [mdff ccc $selall -i open_autopsf.dx -res {}]'.format(resolution),
                             'lappend cc $lcc $fcc',
    ...
    task8.copy_input_data = [
        ...
        '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, sixth_stage.uid,
            task6.uid, 'codh-step1.restart.coor'),

        '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, sixth_stage.uid,
            task6.uid, 'codh-step1.restart.vel'),

        '$Pipeline_{}_Stage_{}_Task_{}/{}'.format(p.uid, sixth_stage.uid,
            task6.uid, 'codh-step1.restart.xsc'),
    ...
    task8.copy_output_data = [
            'codh-step1.restart.coor > $SHARED/{}_{}_codh-step1.restart.coor'.format(rep_idx, iter_idx),
            'codh-step1.restart.vel > $SHARED/{}_{}_codh-step1.restart.vel'.format(rep_idx, iter_idx),
            'codh-step1.restart.xsc > $SHARED/{}_{}_codh-step1.restart.xsc'.format(rep_idx, iter_idx),
